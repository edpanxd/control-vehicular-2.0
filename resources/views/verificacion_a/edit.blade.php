    ...
    <form action="/verificacion_a/{{ $valores->id }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="form-group col-md-6">
                ...
                <select class="custom-select select2" id="pe" name="vehiculo" required>
                    @foreach ($datos as $datos)
                        <option value="{{ $datos->id_vehiculo }}">{{ $datos->id_vehiculo }}, {{ $datos->marca }},
                            {{ $datos->serie }}</option>
                    @endforeach
            ...
            <div class="form-group col-md-6">
                ...
                <input type="date" class="form-control" name="fecha_pago" value="{{ $valores->fecha_pago }}"
                    maxlength="20" required>
            </div>

            <div class="form-group col-md-6">
                <label for="message-text" class="col-form-label">Fecha próximo pago:</label>
                <input type="date" class="form-control" name="fecha_estimada" value="{{ $valores->fecha_estimada }}"
                    maxlength="20" required>
            </div>
            <div class="form-group col-md-6">
                <label for="message-text" class="col-form-label">Monto:</label>
                <input type="number" step="any" class="form-control" name="monto" value="{{ $valores->monto }}"
