<x-jet-action-section>
    <x-slot name="title">
        {{ __('Autenticacion de dos pasos') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Añade seguridad a tu cuenta verificandola por dos pasos.') }}
    </x-slot>

    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                {{ __('Tienes activa la verificacion por dos pasos.') }}
            @else
                {{ __('No tienes activa la verificacion por dos pasos.') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Con la verificacion de dos pasos activa puedes contar con una mayor seguridad en tu cuenta.') }}
            </p>
        </div>
